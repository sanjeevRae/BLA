...
export default function App() {
  ...
  function openAddressSearch(address) {
    if (!address?.trim()) {
      setOrderMessage('Enter an address first.');
      return;
    }
  ...
  function fillCurrentLocation(prefix) {
    if (!navigator.geolocation) {
      setOrderMessage('Location is not supported in this browser.');
      return;
    }

    setLocationBusy(prefix);
    setOrderMessage('');

    navigator.geolocation.getCurrentPosition(
      (position) => {
        updateForm(`${prefix}_latitude`, position.coords.latitude.toFixed(6));
        updateForm(`${prefix}_longitude`, position.coords.longitude.toFixed(6));
        setOrderMessage(`Filled ${prefix} coordinates from current location.`);
        setLocationBusy(null);
      },
      (error) => {
        setOrderMessage(`Could not get current location: ${error.message}`);
        setLocationBusy(null);
      },
      {
                ...
                <form onSubmit={submitOrder} className="space-y-3 border-t border-slate-100 pt-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => openAddressSearch(orderForm.delivery_address)}
                      ...
                      <button
                        type="button"
                        onClick={() => fillCurrentLocation('delivery')}
                        disabled={locationBusy === 'delivery'}
                        className="rounded border border-slate-300 px-2 py-1 text-xs text-slate-700 hover:bg-slate-100 disabled:opacity-50"
                      >
                        {locationBusy === 'delivery' ? 'Getting location…' : 'Use current location'}
                      </button>
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => openAddressSearch(orderForm.pickup_address)}
                        className="rounded border border-slate-300 px-2 py-1 text-xs text-slate-700 hover:bg-slate-100"
                      >
                        Open in map
                      </button>
                      <button
                        ...
                        disabled={locationBusy === 'pickup'}
                        className="rounded border border-slate-300 px-2 py-1 text-xs text-slate-700 hover:bg-slate-100 disabled:opacity-50"
                      >
                        {locationBusy === 'pickup' ? 'Getting location…' : 'Use current location'}
                      </button>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <input
